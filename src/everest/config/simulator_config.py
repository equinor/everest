...
""",
    ...
    keep_qsub_output: Optional[int] = Field(default=0, description="Set to 1 to keep error messages from qsub. Usually only to be used if somethign is seriously wrong with the queue environment/setup.")
    submit_sleep: Optional[float] = Field(default=0.5, description="To avoid stressing the TORQUE/PBS system you can instruct the driver to sleep for every submit request. The argument to the SUBMIT_SLEEP is the number of seconds to sleep for every submit, which can be a fraction like 0.5")
    queue_query_timeout: Optional[int] = Field(default=126, description="""
...
""")
    project_code: Optional[str] = Field(default=None, description="String identifier used to map hardware resource usage to a project or account. The project or account does not have to exist.")
